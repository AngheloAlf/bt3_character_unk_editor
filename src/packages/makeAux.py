...
def getObjects():
    pyFiles = [f[:-3]
              ...
              if os.path.isfile(os.path.join(os.getcwd(), f)) and f.lower().endswith(".py")
               and f != "__init__.py" and f != "makeAux.py"]
    cFiles = [f[:-2]
              for f in os.listdir(os.path.join(os.getcwd(), "c_libraries"))
              if os.path.isfile(os.path.join(os.getcwd(), "c_libraries", f))
              and f.lower().endswith(".c") and f != "main.c"]
    packages = set(pyFiles) - set(cFiles)
    packages = list(packages)
    packages.sort()
    packages = [f+"."+getExtension() for f in packages]
